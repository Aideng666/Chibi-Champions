        ...
        if (CheckWaveComplete())
        {
            ...
            if (!beginWaveAlertFired)
            {
                if (FindObjectOfType<UDPClient>() != null)
                {
                    ...
                    {
                        AlertManager.Instance.DisplayAlert(new Alert(Color.red, $"Prepare Your Defences! When Ready, Press Q {currentWave + 1}", 1000));
                    }
                    else
                    {
                        print($"Player Num: {PlayerClient.Instance.GetClientNum()}");
                        AlertManager.Instance.DisplayAlert(new Alert(Color.red, $"Prepare Your Defences! Waiting For Player 1 To Start Wave {currentWave + 1}", 1000));
                    }
                }
                else
                {
